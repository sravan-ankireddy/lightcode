...
class BERT(nn.Module):
	def __init__(self, mod, input_size, m, d_model, N, heads, dropout, custom_attn=True, attn_mode = "no_attn", pos_mode = "no_pos", multclass = False, NS_model=0):
		super(BERT, self).__init__()
		self.mod = mod
		self.multclass = multclass
		self.m = m
		self.encoder = Encoder(mod, d_model, N, heads, dropout, input_size, custom_attn, attn_mode, pos_mode, NS_model)
		if mod == "trx":
...
class Encoder(nn.Module):
	def __init__(self, mod, d_model, N, heads, dropout, input_size, customAttn= True, attn_mode = "no_attn", pos_mode = "no_pos", NS_model = 0):
		super(Encoder, self).__init__()
		self.mod = mod
		self.Num_layers = N
		self.dropout = nn.Dropout(dropout)
		self.layers = get_clones(EncoderLayer(d_model, heads, dropout, custom_attn = customAttn, attn_mode = attn_mode), N)
...
class EncoderLayer(nn.Module):
	def __init__(self, d_model, nhead, flag_TX,  dropout=0.1, norm_first = True, custom_attn = True, attn_mode = "no_attn"):
